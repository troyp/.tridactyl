 ...
 * Contact: allan.jardine /AT\ sprymedia.co.uk
 */
if(typeof Unit!="undefined"){Unit=undefined}var Unit={oFirstClick:null,oSecondClick:null,_oDrag:{nTarget:null,iNodeStartX:null,iNodeStartY:null,iMouseStartX:null,iMouseStartY:null},oState:{sX:"50px",sY:"40px"},nDisplayTriangle:null,sColour:"blue",fnInit:function(){this.iBodyHeight=document.body.offsetHeight;
...
jsCore.addListener({mElement:document,sType:"mousedown",fnCallback:this.fnMouseClick,oObj:this,mScope:true,sLabel:"Unit"});
...
};jsCore.addListener({mElement:window,sType:"resize",fnCallback:function(){Unit.fnReInit.call(Unit)
},sLabel:"Unit"})},fnClose:function(){if(typeof Design!="undefined"){document.getElementById("UnitSwitch").className="level2";
Design.oState.bUnit=false;Design.fnSaveState()}if(this.nDisplayTriangle!=null){document.body.removeChild(this.nDisplayTriangle);
document.body.removeChild(this.nDisplayHoriz);document.body.removeChild(this.nDisplayVert)
}document.body.style.cursor="auto";jsCore.removeListener({sLabel:"Unit"});try{document.body.removeChild(this.nCanvas)
}catch(a){}try{document.body.removeChild(this.nFirstClickIndicator)}catch(a){}document.body.removeChild(this.nInfo);
Unit=undefined},fnReInit:function(){this.iBodyHeight=document.body.offsetHeight;this.iBodyWidth=document.body.offsetWidth;
...
},fnSaveState:function(){DesignCore.fnCreateCookie("SpryMediaUK_Unit",DesignCore.toJsonString(this.oState))
},fnGetState:function(){try{var oObj=eval("("+DesignCore.fnReadCookie("SpryMediaUK_Unit")+")");
...
if(a=="UnitInfo_Close"){this.fnClose();return}else{if(a=="UnitInfo_DragHandle"){this._oDrag.nTarget=this.nInfo;
this._oDrag.iNodeStartX=this._oDrag.nTarget.offsetLeft;this._oDrag.iNodeStartY=this._oDrag.nTarget.offsetTop;
this._oDrag.iMouseStartX=b.pageX;this._oDrag.iMouseStartY=b.pageY;jsCore.addListener({mElement:document,sType:"mouseup",fnCallback:this.fnDragHandleRelease,oObj:this,mScope:true,sLabel:"Unit"});
jsCore.addListener({mElement:document,sType:"mousemove",fnCallback:this.fnDragHandleMove,oObj:this,mScope:true,sLabel:"Unit"});
...
document.body.removeChild(this.nDisplayHoriz);document.body.removeChild(this.nDisplayVert)
}else{document.getElementById("UnitInfo_Info").style.display="none";document.getElementById("UnitInfo_Difference").style.display="block"
}this.oInfoNodes.nDiff.innerHTML="0px";this.oInfoNodes.nDiffX.innerHTML="0px";this.oInfoNodes.nDiffY.innerHTML="0px";
this._oDrag.iMouseStartX=b.pageX;this._oDrag.iMouseStartY=b.pageY;this.nDisplayTriangle=DesignCore.fnCreateBox({sTop:b.pageY+"px",sLeft:b.pageX+"px",sWidth:"0px",sHeight:"0px",fOpacity:0.3});
this.nDisplayHoriz=DesignCore.fnCreateBox({sId:"UnitRuleHoriz",sTop:(b.pageY-5)+"px",sLeft:b.pageX+"px",sWidth:"0px",sHeight:"11px"});
this.nDisplayVert=DesignCore.fnCreateBox({sId:"UnitRuleVert",sTop:b.pageY+"px",sLeft:(b.pageX-5)+"px",sWidth:"11px",sHeight:"0px"});
...
jsCore.addListener({mElement:document,sType:"mouseup",fnCallback:this.fnMouseUp,oObj:this,mScope:true,sLabel:"Unit"});
jsCore.addListener({mElement:document,sType:"mousemove",fnCallback:this.fnMouseMove,oObj:this,mScope:true,sLabel:"Unit"})
...
},fnMouseMove:function(f){var b=f.pageX-this._oDrag.iMouseStartX;var d=f.pageY-this._oDrag.iMouseStartY;
...
this.nDisplayVert.style.backgroundPosition="bottom center"}},fnDragHandleMove:function(a){this._oDrag.nTarget.style.left=(this._oDrag.iNodeStartX+a.pageX-this._oDrag.iMouseStartX)+"px";
this._oDrag.nTarget.style.top=(this._oDrag.iNodeStartY+a.pageY-this._oDrag.iMouseStartY)+"px"
...
},fnDrawHtml:function(){this.nInfo=DesignCore.fnCreateBox({sId:"UnitInfo",sClass:"SpryMedia_Design",sPosition:"fixed",sTop:this.oState.sY,sLeft:this.oState.sX});
document.body.insertBefore(this.nInfo,document.body.childNodes[0]);var c='			<div class="level1">				<div id="UnitInfo_DragHandle"></div>				<div id="UnitInfo_Close"></div>				<div id="UnitInfo_Measurement" class="level2">					<div id="UnitInfo_Info" class="level3">						<div class="level4">							<div class="level5">								<div id="UnitInfo_InfoMessage" class="center">Click and drag between two points to measure between</div>								<div class="clear"></div>								<div class="level6"></div>							</div>						</div>					</div>										<div id="UnitInfo_Difference" class="level3">						<div class="level4">							<div class="level5">								<div class="split1">Difference</div>								<div id="UnitInfo_Diff" class="split2">0px</div>								<div class="clear"></div>																<div class="split1">X difference</div>								<div id="UnitInfo_XDiff" class="split2">0px</div>								<div class="clear"></div>																<div class="split1">Y difference</div>								<div id="UnitInfo_YDiff" class="split2">0px</div>								<div class="clear"></div>																<div class="clear"></div>								<div class="level6"></div>							</div>						</div>					</div>				</div>			</div>';
this.nInfo.innerHTML=c;if(navigator.userAgent.indexOf("WebKit")!=-1){var b=RegExp("( AppleWebKit/)([^.+ ]+)").exec(navigator.userAgent);
var a=b[2].split(".")[0]*1;if(a<523){this.nInfo.style.position="absolute"}}this.oInfoNodes={nDiffBlock:document.getElementById("UnitInfo_Difference"),nDiff:document.getElementById("UnitInfo_Diff"),nDiffX:document.getElementById("UnitInfo_XDiff"),nDiffY:document.getElementById("UnitInfo_YDiff")}
},fnDesignElement:function(a){while(a.parentNode){if(a.className=="SpryMedia_Design"){return true
}a=a.parentNode}return false}};Unit.fnInit();if(document.getElementById("LoadingUnit")){document.getElementById("LoadingUnit").parentNode.removeChild(document.getElementById("LoadingUnit"))
};